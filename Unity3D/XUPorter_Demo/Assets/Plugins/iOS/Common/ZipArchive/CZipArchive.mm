...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		_totalCount = globalInfo.number_entry;
	}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		...
		ret = unzGoToNextFile( _unzFile );
        _extractCount += 1;
        //NSLog([NSString stringWithFormat:@"extractCount = %d file",_extractCount] );
